...
export const updateUserRegister = async (touristId: string, user: IUserRegister) => {
  if (!Types.ObjectId.isValid(touristId)) throw new Error("Invalid tourist ID");

  const tourist = await Tourist.findByIdAndUpdate(touristId, { $push: { userRegister: user } }, { new: true });
